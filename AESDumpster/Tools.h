...
namespace DumpsterTools 
{
  ...
  std::string ConcatenateAESType(std::vector<PBYTE> aes_key_addresses, int i, int type) {
    if (type == 0) { // Binary layout 1
      return
        hexStr(&aes_key_addresses[i][3], 4) + hexStr(&aes_key_addresses[i][10], 4) +
        hexStr(&aes_key_addresses[i][17], 4) + hexStr(&aes_key_addresses[i][24], 4) +
        hexStr(&aes_key_addresses[i][35], 4) + hexStr(&aes_key_addresses[i][42], 4) +
        hexStr(&aes_key_addresses[i][49], 4) + hexStr(&aes_key_addresses[i][56], 4);
    }
    else if (type == 1) { // Binary layout 2
      return
        hexStr(&aes_key_addresses[i][2], 4) + hexStr(&aes_key_addresses[i][9], 4) +
        hexStr(&aes_key_addresses[i][16], 4) + hexStr(&aes_key_addresses[i][23], 4) +
        hexStr(&aes_key_addresses[i][30], 4) + hexStr(&aes_key_addresses[i][37], 4) +
        hexStr(&aes_key_addresses[i][44], 4) + hexStr(&aes_key_addresses[i][51], 4);
    }
    else if (type == 2) { // Binary layout 3
      return
        hexStr(&aes_key_addresses[i][3], 4) + hexStr(&aes_key_addresses[i][10], 4) +
        hexStr(&aes_key_addresses[i][21], 4) + hexStr(&aes_key_addresses[i][28], 4) +
        hexStr(&aes_key_addresses[i][35], 4) + hexStr(&aes_key_addresses[i][42], 4) +
        hexStr(&aes_key_addresses[i][49], 4) + hexStr(&aes_key_addresses[i][56], 4);
    }
    return "";
  ...
  void PrintIntro() {
    std::wcout << "AESDumpster 1.2 - By GHFear @ IllusorySoftware" << std::endl;
